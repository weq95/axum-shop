...
impl AdminAuth {
    ...
    pub async fn permissions_for_role(
        role_name: String,
        domain: String,
    ) -> ApiResult<Vec<CasbinRule>> {
        let enforcer = common::casbin::casbin_layer().await.get_enforcer().clone();
        let enforcer = enforcer.write().await;

        let casbin_rule = match AdminAuth::get_casbin_rule(role_name).await {
            Ok(value) => value,
            Err(e) => {
                return Err(ApiError::Error(e.to_string()));
            }
    ...
    pub async fn permissions_for_user(user_id: u64, domain: String) -> ApiResult<Vec<CasbinRule>> {
        ...
        for role_name in AdminAuth::roles_for_user(user_id, domain.clone()).await? {
            let role_permissions =
                AdminAuth::permissions_for_role(role_name, domain.clone()).await?;
    ...
    pub async fn delete_role_permissions(
        role_permissions: Vec<RolePermissions>,
    ) -> ApiResult<bool> {
